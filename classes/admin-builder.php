...
/**
 * RWC Add Featured Image Admin Settings
 * Requires Genesis 1.8 or later
 *
 * This file registers all of the plugin Settings in the admin/
 * 
 *
 * @package      rwc_add_genesis_cpt_archive_image
 ...
 * @since 1.0.0
 */
function rwc_add_settings() {
    global $_child_theme_settings;
    
    $admin_pages = array();
    $admin_pages = apply_filters ( 'rwc_settings_builder' , $admin_pages );
	...
	$_child_theme_settings = new rwc_add_genesis_cpt_archive_image( $settings, $sanatize, $help, $meta_boxes );
    }
}
add_action( 'admin_menu', 'rwc_add_settings', 5 );

/*
 * url to load local resources.
 */
define( 'RWC_META_BOX_URL', trailingslashit( str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname(__FILE__) ) ) );

 
/**
 * Extends existing genesis CPT archive setting page.
 *
 * @since 1.0.0
 */
class rwc_add_genesis_cpt_archive_image extends Genesis_Admin_Boxes {
 ...
 */
function rwc_scripts() {
  	
		wp_register_script( 'cmb-scripts', rwc_META_BOX_URL.'jquery.cmbScripts.js', array( 'jquery','media-upload','thickbox' ) );
...
function rwc_editor_admin_init() {
	if ( strpos( $_SERVER['REQUEST_URI'], 'post.php' ) OR strpos( $_SERVER['REQUEST_URI'], 'post-new.php' ) OR strpos( $_SERVER['REQUEST_URI'], 'page-new.php' ) OR strpos( $_SERVER['REQUEST_URI'], 'page.php' ) ) {
		...
		wp_enqueue_script( 'editor' );
                ...
                add_action( 'admin_enqueue_scripts', 'rwc_scripts', 10, 1 );
	}
}

function rwc_editor_admin_head() {
...
function rwc_editor_footer_scripts() {  ?>
...
add_action( 'admin_head', 'rwc_styles_inline' );
function rwc_styles_inline() { 
	echo '<link rel="stylesheet" type="text/css" href="' . rwc_META_BOX_URL.'style.css" />';
	?>	
	<style type="text/css">
		...
		table.cmb_metabox .cmb_upload_status .img_status .remove_file_button { text-indent: -9999px; background: url(<?php echo rwc_META_BOX_URL ?>images/ico-delete.png); width: 16px; height: 16px; position: absolute; top: -5px; left: -5px;}
	</style>
	<?php
}